...
We now introduce our notation, summarised in table \ref{tbl:glossary}. % summarises the commonly used symbols.
...
$$ \Sim_{\i' \i} = \frac{ \R_{:\i}^T \R_{:\i'} }{|| \R_{:\i} |s|_2 || \R_{:\i'} ||_2}. $$
...
and interpretable.
%Further, they may also be applied as-is to OC-CF problems \citep{Linden:2003, Deshpande:2004}, unlike most other popular CF models such as matrix factorisation (to be discussed subsequently).
However, they are unable to adapt to the characterstics of the data as they rely on a fixed $\Sim$ that is not learned from data \citep{Koren:2008b}. Furthermore, Recommendation performance can be quite sensitive to the choice of $\Sim$.
...
%
% \subsubsection{Weighted matrix factorisation}
