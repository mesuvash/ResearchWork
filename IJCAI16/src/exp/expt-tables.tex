...
\item \LinearLow is generally competitive with SLIM and outperforms the other baselines on all four datasets. Unlike other methods, performance of \LinearLow is not very sensitive to the choice of user or item based method. In figure~\ref{fig:dimensionPerformance}, we observe that the performance of the \LinearLow method improves with the number of the projection dimensions. 
...
Since similarity metric is an important aspect of personalization, we evaluate item-item similarities learned by I-\LinearLow  model on the \Fotolia dataset. 
