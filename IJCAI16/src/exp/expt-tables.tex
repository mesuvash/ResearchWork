Tables \ref{tbl:guitar-results} -- \ref{tbl:ml10m-results} summarize the results of the various methods.
We make the following observations:
\begin{compactitem}

\item \LinearLow is very competitive with SLIM and consistently outperforms other baselines on all four datasets. Unlike other methods, the performance of \LinearLow is not very sensitive to the choice of user or item based method. 
%In our experiments 
In Figure~\ref{fig:dimensionPerformance}, we observe that the performance of \LinearLow improves steadily  with the dimensionality of projection dimension, but with diminishing returns. However, WRMF performance improves initially with the number of dimensions and then starts to decrease possibly due to overfitting. 
...
\item Amongst the matrix factorization methods, Randomized SVD method~\citep{Tang:2013} consistently performs the best in all dataset. This is possibly due to the fact that SVD provides a good initialization and the optimization finds optimal solution for the given initialization, whereas matrix factorization optimizes a nonconvex bilinear objective with random initialization which may lead to an inferior solution. In Table~\ref{tbl:guitar-results} -- \ref{tbl:lfm-results}, we observe that the performance of MF-RSVD varies significantly based on whether we initialize user or item latent factors.
...
\begin{figure}[!ht]
  ...
  \caption{Variation of performance of I-\LinearLow and WRMF with the number of dimensions.}
...
\begin{table*}[!htb]
	...
	\resizebox{0.85\textwidth}{!}{%
		...
		\toprule
		\multirow{3}{*}{\textbf{Similar items}} & Test and Analysis & Beach Holidays & Construction & Job Search & Depression \\
