...
\item We observe that the neighborhood models yield inferior results compared to the Linear models. Also, the performance varies with the user and item based models. While they perform comparatively competitive in \Lowes\ and \Guitar\ dataset, they perform poorly in \LastFM\ and \MLens\ dataset. Hence, neighborhood methods are not consistent in terms of performance.
...
\begin{figure}[!ht]
  ...
  \caption{Performance of the U-\LinearLow and I-\LinearLow with the number of dimensions.}
...
In Table~\ref{tbl:similarity-evaluation}, we show top-3 similar items for a small subset of items learned by I-\LinearLow  model on \Fotolia dataset.
%for a small subset of items for  qualitatively evaluate the item-item similarity learned by the I-\LinearLow model on \Fotolia dataset. 
 We see that  I-\LinearLow learns very meaningful and explainable similarities making it very applicable in similar items recommendation.
