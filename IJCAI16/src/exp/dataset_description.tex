...
From the original rating matrix $\tilde\R \in \{ 0, 1, \ldots, 5 \}^{\numUsers \times \numItems}$, we created a preference matrix $\R$ with $R_{\ui} = \indicator{\tilde{\R}_{\ui} \geq 4}$. \todob{Note that we report $\indicator{\R_{\ui} > 0}$ in Table \ref{tbl:datasets}. Not $\indicator{\R_{\ui} \geq 4}$.} \todob{I updated the notation now $\R_{ui}$ is binary matrix constructed from $\tilde{\R}_{\ui}$}
